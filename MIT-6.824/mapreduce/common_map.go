...
func doMap(
	...
	if err != nil {
		log.Fatalf("doMap-> open input file error: %v\n", err)
	}

	content, err := ioutil.ReadAll(file)
	if err != nil {
		log.Fatalf("doMap-> read file content error: %v\n", err)
	}

	reduceFiles := make([]*os.File, nReduce)
	for i := 0; i < nReduce; i++ {
		reduceFileName := reduceName(jobName, mapTaskNumber, i)
		reduceFiles[i], err = os.Create(reduceFileName)
		if err != nil {
			log.Fatalf("doMap-> create file error: %v\n", err)
		}
	}
	...
	for i := range reduceFiles {
		encArr[i] = json.NewEncoder(reduceFiles[i])
	}

	kvs := mapF(inFile, string(content))
	for i := range kvs {
		r := ihash(kvs[i].Key) % nReduce
		encArr[r].Encode(&kvs[i])
	}
}
